...
export function WorkerGrid({ className }: WorkerGridProps) {
  ...
  const handleFixError = useCallback(
    async (error: ValidationError) => {
      ...
      try {
        const suggestions = ErrorFixEngine.getFixSuggestions(error, {
          clients,
          workers,
          tasks,
        });
        const autoFixable = suggestions.find(
          s => s.canAutoFix && s.confidence === 'high'
        );

        if (autoFixable) {
          const result = await ErrorFixEngine.applyFix(error, autoFixable.id, {
            clients,
            workers,
            tasks,
          });
    ...
    [clients, workers, tasks, setWorkers]
  );
  ...
  const handleBulkFix = useCallback(
    async (errors: ValidationError[]) => {
      setIsFixingErrors(true);
      try {
        const result = await ErrorFixEngine.applyBulkFix(errors, {
          clients,
          workers,
          tasks,
        });
    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: (params: { value: string | number }) => {
          ...
          return (
            <Badge
              className={colors[level as keyof typeof colors] || 'bg-gray-100'}
            >
              {level}
            </Badge>
          );
  ...
  const onCellValueChanged = useCallback(
    (event: CellValueChangedEvent<Worker>) => {
  ...
  const getRowClass = useCallback(
    (params: { node: { rowIndex: number | null } }) => {
      ...
      const hasError = workerErrors.some(
        error =>
          error.rowIndex === params.node.rowIndex && error.level === 'error'
      );
      const hasWarning = workerErrors.some(
        error =>
  ...
  return (
    <Card className={className}>
      ...
      <CardContent>
        ...
        {showValidationDetails && workerErrors.length > 0 && (
          <div className="mt-6">
